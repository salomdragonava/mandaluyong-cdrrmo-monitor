...
def action_status(level,flood_status,pred,candidates,elevated_escape):
    risk=pred.get('risk','UNKNOWN'); imminent=bool(pred.get('imminent'))
    if level=='RED' or flood_status=='HIGH RISK' or imminent:
        return '🔴 LEAVE','Flood conditions may be imminent. Move to a safe location and do not wait for water to rise further.'
    if level=='ORANGE' or flood_status=='ESCAPE ROUTE ELEVATED' or risk=='HIGH' or candidates:
        return '🟠 PREPARE TO LEAVE','Flood risk is increasing. Prepare essentials, secure belongings, and be ready to leave if conditions worsen.'
    if level=='YELLOW' or flood_status=='WATCH' or risk in {'WATCH','LOW-MODERATE'} or elevated_escape:
        return '🟡 PREPARE','Conditions need closer attention. Prepare essentials and monitor the next updates.'
    return '🟢 SAFE','No immediate action is needed. Conditions are currently stable.'
def build_messages():
    pagasa=load(PAGASA_STATE,{}); flood=load(FLOOD_STATE,{}); radar=load(RADAR_STATE,{}); assess=load(ASSESSMENT_STATE,{}); previous=load(MONITOR_STATE,{}); level=pagasa.get('level','UNKNOWN'); flood_status,rainfall,elevated_escape=flood_summary(flood); candidates=radar_candidates(radar); now=datetime.now(PH_TZ); pred=assess.get('prediction',{}); action,action_text=action_status(level,flood_status,pred,candidates,elevated_escape); lines=[f'{warning_icon(level)} MANDALUYONG FLOOD MONITOR','',f'{action}','{action_text}','',f'Checked: {now.strftime("%Y-%m-%d %I:%M %p")}','','🌧️ WEATHER',f'PAGASA: {warning_icon(level)} {level}',warning_description(level),'', '📍 LOCAL MONITORING',f'Flood status: {flood_status}']
    ...
    alert_text='\n'.join(lines); ALERT_FILE.write_text(alert_text,encoding='utf-8')
