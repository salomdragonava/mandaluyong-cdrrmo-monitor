...
def main():
    ...
    if candidates:
        ...
        lines = [
            '🔴 RED CORE / HEAVY RAIN RADAR ALERT — MANDALUYONG',
            '',
        ...
        status_line = '🚨 Significant radar core detected near/approaching Mandaluyong.'
    else:
        lines = [
            '🟢 No Red Core or Heavy Rain Detected',
            '',
            f'PAGASA radar frame: {current_ts or "unknown"}',
            'No qualifying red/orange/purple radar core met the alert criteria for Mandaluyong.',
        ]
        status_line = 'Monitoring continues hourly.'

    lines += [
        '',
        status_line,
        'Automated radar signal only; not an official PAGASA rainfall warning.',
        f"Checked: {datetime.now(PH_TZ).strftime('%Y-%m-%d %H:%M:%S %Z')}",
    ]
    ALERT_FILE.write_text('\n'.join(lines), encoding='utf-8')
    NOTIFY_STATE.write_text(json.dumps({'last_checked_timestamp': current_ts, 'last_alert_timestamp': current_ts if candidates else None}, indent=2), encoding='utf-8')
